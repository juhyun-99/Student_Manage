...
<%
	...
	String result1 = "";
	StudentDO sDO= (StudentDO) session.getAttribute("student");
	if(sDO != null){
		result1 += "<tr><td>" + Integer.toString(sDO.getNum()) +"</td><td>" +  sDO.getName() +"</td><td>"+  sDO.getGender() + "</td><td>" + 
				Integer.toString(sDO.getKorScore()) +"</td><td>" + Integer.toString(sDO.getEngScore()) +"</td><td>"+ 
				Integer.toString(sDO.getMathScore()) +"</td><td>" +   Integer.toString(sDO.getSciScore()) + "</td></tr>";
	}
%>
...
<body>
	...
	<form method = "POST">
	<fieldset>
		<legend>정보를 조회할 학생의 학번을 입력하세요</legend>
		<label for = "num">학번</label>
		<input type = "text" name = "num" id = "num" size = "10"/>
		<input type="hidden" name="command" value="studentSearch" />
